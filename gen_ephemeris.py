...
def main(time, parfile, outparfile, outtimfile, time_cut, threshold=0.16):
    #fsearch
    fbest, mjd, ferr = fsearch(time, parfile, duration=3600*10, fig_flag=False,
            fstep=1e-6, frange=1e-4, bin_profile=50, mission='fermi',threshold=threshold)
    ...
    #calculate ToAs
    toa = toa_cal(time, outparfile, duration=3600*10, bin_profile=50, std_pro_file='', fig_flag=False, mission='FERMI',threshold=threshold)
    write_toa(outtimfile, toa, 'Fermi', error=1500)
...
if __name__ == '__main__':
    datafile = '../Vela/vela_bary.fits' #TODO:KeyboardInput
    rawparfile = 'testout_10.par'
    newparfile_prefix = 'testout'
    newtimfile_prefix = 'testout'
    mission = 'FERMI'
    time_cut = 30
    time_list = read_file(datafile, colname='Time', time_cut=time_cut) 

    for time, i in zip(time_list, xrange(len(time_list)))[11:]:#XXX:temporary test
        ...
        if i == 11:
            lastparfile = main(time, rawparfile, outparfile, outtimfile, time_cut)
        else:
            lastparfile = main(time, lastparfile,  outparfile, outtimfile, time_cut)
        plt.show()
