...
def le_profile_fitting_ti(pathlist, filename, parfilename, std_pro,duration=300, f3_flag=True, f4_flag=True, fig_flag=False,dtime_flag=False,psf_flag=False):
    ...
    for path in pathlist:
        ...
        try:
            ...
            detbox = tb.field('Board')
            print path
        except:
            print "NO TDB column"
            continue 
        print 'select DetBox == 2'
        tdb_all = tdb_all[np.where(detbox == 2)[0]]
        time_all = time_all[np.where(detbox == 2)[0]]
        if len(tdb_all) == 0:continue
        hdulist.close()
        ...
        ############# psf correction ################
            if psf_flag:
#                time_qt, psf1, psf2, psf3 = projection_cor_le(path)
                time_qt, psf3 = projection_cor_le(path)
#                psf1_frac = np.mean(psf1[(time_qt >= edges[i]) & (time_qt <= edges[i+1])])
#                psf2_frac = np.mean(psf2[(time_qt >= edges[i]) & (time_qt <= edges[i+1])])
