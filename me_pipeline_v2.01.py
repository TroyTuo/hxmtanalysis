...
def main():
    ...
    ## select detectors
    det = '0-5,7,12-23,25,30-41,43,48-53,10,28,46'
    print 'small FoV detector list:',det
    mescreen_text = 'mescreen evtfile='+tmp_dir+'me_grade.fits gtifile='+tmp_dir+'me_gti.fits outfile='+clean_dir+'me_screen.fits '+\
    ...
    spec_text = 'mespecgen evtfile="'+clean_dir+'me_screen.fits" outfile="'+\
            ...
            '0-5,7,12-23,25,30-41,43,48-53" starttime=0 stoptime=0 minPI=0 maxPI=1024' 
    print spec_text
    os.system(spec_text)
    ...
    mebkgmap(product_dir, clean_dir+'me_screen.fits', ehkfilename, tmp_dir+'me_gti.fits', tmp_dir+'me_dtime.fits',
            tempfilename)
    melcgen(lc_dir, clean_dir+'me_screen.fits', clean_dir+'me_screen.fits', tmp_dir+'me_dtime.fits', ehkfilename, tmp_dir+'me_gti.fits',
...
def melcgen(lc_dir, screenfile, blindfile, deadfile, ehkfile, gtifile, tempfile, binsize=1, minPI=100, maxPI=600):
    melc_text = 'melcgen evtfile="'+screenfile+'" outfile="'+lc_dir+'me_lc'+'" deadfile="'+deadfile+\
...
def merspgen(product_path, attfile, ra, dec):
    phafile = glob.glob(product_path+"/ME/spectra/me_spec_g*")[0]
    outfile = os.path.join(product_dir,'ME','rsp','me_rsp.fits')
    attfile = attfile
    ra = str(ra)
    dec = str(dec)
    rsp_text = "merspgen %s %s %s %s %s clobber=yes"%(phafile, outfile, attfile, ra, dec)
    print(rsp_text)
    os.system(rsp_text)
