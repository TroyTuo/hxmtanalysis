...
for i in xrange(len(low_edge)):
    ...
    f.write(wrt_str)
f.close()
## select evttype=1 time range
fselect_text = 'fselect '+filename+' '+le_dir+'le_evt.fits @'+le_dir+'evttype.tim clobber=yes'
print fselect_text
os.system(fselect_text)
## select evttype=1 photons
fselect_text = 'fselect '+le_dir+'le_evt.fits '+le_dir+'le_evt.fits "Event_Type==0" clobber=yes'
print fselect_text
os.system(fselect_text)
...
' defaultexpr=no ELV=6 dye_elv=40 COR=1 sun_angle=10 '+\
